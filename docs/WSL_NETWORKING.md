...
UDP-мост VST (Windows) ↔ engine (WSL) **не использует звуковой драйвер Windows** для PCM между машинами. Reaper/WASAPI влияют только на последний шаг (вывод на колонки). Рваный звук чаще всего даёт **виртуальная сеть WSL2**, а не драйвер Realtek/USB.
