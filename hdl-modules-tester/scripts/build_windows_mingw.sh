...
SECONDS=0
verilator --cc --exe generator.sv "${SOURCES[@]}" \
  --Mdir "$OBJ_DIR" \
  --top-module generator \
  -CFLAGS "-I$TT_ROOT -D_WIN32_WINNT=0x0A00" \
  -LDFLAGS "-lws2_32 -static" \
  -o Vgenerator.exe

make -C "$OBJ_DIR" -f Vgenerator.mk \
  CXX="$CXX" AR="$AR" \
  engine.o main.o net_socket.o synth_core.o verilated.o \
  Vgenerator__ALL.a
...
echo
echo "Windows engine: $TT_ROOT/$EXE"
echo
