...
std::thread startUdpInput(const UdpInputConfig& cfg, SharedState& state, UdpSessionState& session) {
    return std::thread([cfg, &state, &session]() {
        UdpSocket sock;
        if (!sock.open()) {
            std::cerr << "UDP input socket open failed\n";
            return;
        }
        if (!sock.bind(cfg.bindHost, cfg.controlPort)) {
        ...
        while (state.running.load(std::memory_order_relaxed)) {
            UdpEndpoint src;
            const ssize_t n = sock.recvFrom(buffer.data(), buffer.size(), src);
            if (n <= 0) {
            ...
            case hdlnet::PacketType::Hello: {
                ...
                sock.sendTo(out.data(), out_len, dest);
            ...
            case hdlnet::PacketType::Ping: {
                ...
                sock.sendTo(out.data(), out_len, UdpEndpoint{src.host, src.port});
                break;
            }
