...
void printUsage() {
    std::cerr
        << "VgeneratorFull legacy (keyboard/MIDI → soundcard/wav, full MIDI note range)\n"
        << "Options:\n"
...
int main(int argc, char** argv) {
    ...
    SynthCore synth;
    const int synthChannels = outputMode == OutputMode::Soundcard ? 2 : 1;
    ...
    const char* inputLabel = inputSource == InputSource::Midi ? "midi" : "keyboard";
    const char* outputLabel = outputMode == OutputMode::Soundcard ? "soundcard" : "wav";

    std::cerr << "VgeneratorFull | Input: " << inputLabel
              << " | Output: " << outputLabel
              << " | Press [x] to quit\n";
